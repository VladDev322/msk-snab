...
const accordionOptions = { alwaysOpen: true };

accordions.forEach(accordion => new ItcAccordion(accordion, accordionOptions));
...
let tabs2 = document.querySelectorAll('.register__menu-link');
let datas2 = document.querySelectorAll('.auth__form');

tabs2.forEach(tab => {
  tab.addEventListener('click', function (element) {

    // active necessary tab
    tabs2.forEach(tab => tab.classList.remove('register__menu-link--active'));
    element.target.classList.add('register__menu-link--active');

    // show necessary data
    let btn = element.target.getAttribute('data-face');
    datas2.forEach(data => data.classList.remove('auth__form--active'));
...
form.addEventListener('submit', function (event) {
  ...
  fetch('/submitForm', {
    ...
    .catch(error => {
      // Отображение модального окна с ошибкой
      showModal(0);
    });
});
...
window.addEventListener('click', function (event) {
  ...
  else console.log(event.target, resultModal);
});
...
function showModal(result) {
  ...
  if (result == 1) {
      modal.innerHTML =
  `<img src="sourse/svg/success.svg" alt="" class="modal__img">
  <p class="modal__text">заявка отправлена</p>
  <p class="modal__description">В ближайшее время наш менеджер свяжется с вами</p>`;
  } else 
  if (result == 0) {
    modal.innerHTML =
    `<img src="sourse/svg/error.svg" alt="" class="modal__img">
    <p class="modal__text">что-то пошло не так...</p>
    <p class="modal__description">Попробуйте еще раз</p>`;
  }
  document.body.appendChild(modal);
  // Закрытие модального окна через некоторое время (например, 3 секунды)
  setTimeout(function () {
